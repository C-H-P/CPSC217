...
from CPSC217S21A3BoardBonus import *
...
def evaluate(board: Board, player1: Piece, player2: Piece,pieces_to_win) -> int:
    """
    ...
    """
    if won(board, player1,pieces_to_win):
        score = 1
    elif won(board, player2, pieces_to_win):
        score = -1
    else:
        score = 0
    return score
...
# Newly added parameter: number of pieces in a row to win
def minimax(board: Board, player1: Piece, player2: Piece, player: Piece, depth: int, pieces_to_win) -> List:
    """
    ...
    if depth == 0 or gameover(board,pieces_to_win):
        score = evaluate(board, player1, player2,pieces_to_win)
    ...
    for move in moves:
        ...
        score = minimax(board, player1, player2, next_player, depth - 1, pieces_to_win)
...
# Newly added parameter: number of pieces in a row to win
def AI(board: Board, player1: Piece, player2: Piece, level: int, pieces_to_win) -> Tuple[Row, Col]:
    """
    ...
    if level >= 4:
        result = minimax(board, player1, player2, player1, rowsIn(board) * colsIn(board) + 1, pieces_to_win)
        return result[0], result[1]
    # Otherwise multiply level by two (to get plays by each side for each level)
    elif level > 0:
        result = minimax(board, player1, player2, player1, level * 2, pieces_to_win)
        return result[0], result[1]
    # Otherwise random
    moves = openMoves(board)
    for move in moves:
...
# prompt for getting number of consecutive pieces to win the game
GET_NUM_PROMPT = """Number of pieces in a row to win:
\t3
\t4
\t5
"""

# prompt to get number of pieces in a row to win and exit if invalid value is entered
# parameter: current board state in the format of 2D list
# returns integer
def getValidPiecesToWin(board:Board)->int:
    print(GET_NUM_PROMPT,end = "")
    pieces_to_win = int(input("Pick number of pieces in a row to win(less than or equal to the length/width of the board)):"))

    # Check to make sure the value user entered is valid, otherwise, keep looping until the user enter the correct value
    if pieces_to_win > len(board) and pieces_to_win > len(board[0]):
        print("Invalid: number of pieces to win cannot fit in the board.")
        sys.exit()
...
# Newly added parameter: number of pieces in a row to win
def getAndDrawHint(board: Board, human: Piece, computer: Piece, h: str, pieces_to_win) -> None:
    """
    ...
    if h == "h":
        print("Wait for hint")
        row1, col1 = hint(board, human,pieces_to_win)
        row2, col2 = hint(board, computer,pieces_to_win)
        if row1 != -1:
        ...
        if row1 != -1:
            ...
            row, col = AI(board, human, computer, 4,pieces_to_win)
        else:
            print("Wait for hint (slow)")
            row, col = AI(board, human, computer, 3,pieces_to_win)
        if row != -1:
...
def main() -> None:
    ...
    # Get number of pieces in a row to win
    pieces_to_win = getValidPiecesToWin(board)
    ...
    if input("Enter G to enter input with mouse, otherwise use shell") == "G":
        gui_flag = True
    while not gameover(board,pieces_to_win):
        ...
        if human == player:
            ...
            # Get and draw hint
            getAndDrawHint(board, human, computer, h,pieces_to_win)
            ...
            row, col = AI(board, computer, human, difficulty_input, pieces_to_win)
            play(board, row, col, computer)
            print(f"AI plays in row={row} and col={col}")
            player = human
        drawBoard(board)
        plays += 1
    handleGameOverDrawing(board, human,pieces_to_win)
    handleExit()

# Newly added parameter: number of pieces in a row to win
def handleGameOverDrawing(board: Board, human: Piece, pieces_to_win) -> None:
    """
    ...
    if won(board, X,pieces_to_win):
        ...
        setColor("black")
        text(300, 300, "X won!")
    elif won(board, O, pieces_to_win):
        if human == O:
            drawBoard(board, WIN_COLOR)
        else:
            drawBoard(board, LOSE_COLOR)
